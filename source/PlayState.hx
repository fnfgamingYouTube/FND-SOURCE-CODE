...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Microwave', 0.2], //From 0% to 19%
		['Pig', 0.4], //From 20% to 39%
		['Racist', 0.5], //From 40% to 49%
		['Bruh', 0.6], //From 50% to 59%
		['One More Chance', 0.69], //From 60% to 68%
		['Noice', 0.7], //69%
		['lol', 0.8], //From 70% to 79%
		['Whitelisted', 0.9], //From 80% to 89%
		['Good Soldier', 1], //From 90% to 99%
		['MULTI POWERFUL', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'room1': //Week 7 - King
				{
					defaultCamZoom = 0.9;
					curStage = 'room1';
					var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('room1'));
					bg.antialiasing = FlxG.save.data.antialiasing;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);
				}
			case 'room2': //Week 7 - Hell
				{
					defaultCamZoom = 0.9;
					curStage = 'room2';
					var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('room2'));
					bg.antialiasing = FlxG.save.data.antialiasing;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);
				}
			case 'room3': //Week 8 - Dramas
				{
					defaultCamZoom = 0.9;
					curStage = 'room3';
					var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('room3'));
					bg.antialiasing = FlxG.save.data.antialiasing;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);
